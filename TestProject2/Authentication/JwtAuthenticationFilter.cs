...
namespace TestProject2.Authentication
{
    /// <summary>
    ...
    public class JwtAuthenticationFilterAttribute : Attribute, IAuthenticationFilter
    {
        ...
        private async Task<IPrincipal> ValidatecredAsync(string cred , HttpRequestMessage  requestMessage, CancellationToken canc) {


            var tokenHandler = new JwtSecurityTokenHandler();

           if (tokenHandler.CanReadToken(cred))
                return null;


            TokenValidationParameters validationpara = new TokenValidationParameters
            {
                ValidateAudience = true,
                ValidAudiences = new[] { _audience },
            ...
            SecurityToken validatedsectoken = new JwtSecurityToken();
              ClaimsPrincipal claimsPrincipal =  tokenHandler.ValidateToken(cred, validationpara, out validatedsectoken);



            ((ClaimsIdentity)claimsPrincipal.Identity).AddClaim(new Claim("urn:Issuer",
               validatedsectoken.Issuer));
            ((ClaimsIdentity)claimsPrincipal.Identity).AddClaim(new Claim("urn:TokenScheme",
                SupportedTokenScheme));
            ...
            // store it in a standard claim name or the bootstrap context 
            // for later retrieval by the other filters/action methods
            ((ClaimsIdentity)claimsPrincipal.Identity).BootstrapContext = cred;
